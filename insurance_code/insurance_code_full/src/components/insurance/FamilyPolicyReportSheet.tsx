...
import { api, type FamilyPolicyReportResponse, type InsurancePolicy } from '../../lib/api';
import {
              ...
              <td className="px-4 py-4">
                <div className="flex flex-wrap gap-2">
                  {member.protectionTypes.map((type) => (
                    <Tag key={`${member.name}-${type}`}>{type}</Tag>
                  ))}
                </div>
              </td>
              ...
              <td className="px-4 py-4">
                <div className="flex flex-wrap gap-2">
                  {applicant.protectionTypes.map((type) => (
                    <Tag key={`${applicant.name}-${type}`}>{type}</Tag>
                  ))}
                </div>
              </td>
...
function insuranceNeedGuidance(label: string) {
  if (label === '健康') return '重点确认重疾给付和医疗报销是不是同时具备，而不是只配到其中一层。';
  if (label === '人寿') return '重点确认主要收入承担者的身故 / 全残责任是否足以承接家庭支出。';
  if (label === '养老') return '重点确认长期现金流安排是否已经开始，而不是只看短期保障。';
  if (label === '财富') return '重点确认教育金、传承或长期储备目标是否已经有专门承接。';
  if (label === '意外') return '重点确认高频出行、老人和孩子的意外责任是不是有基础覆盖。';
  return '结合家庭目标再确认这一层需求是否需要优先补位。';
}
...
function InsuranceNeedRadarChart({ axes }: { axes: InsuranceNeedAxis[] }) {
  ...
  const strongest = [...points].sort((a, b) => b.score - a.score || b.coverage - a.coverage)[0];
  const weakest = [...points].sort((a, b) => a.score - b.score || a.coverage - b.coverage)[0];
    ...
    <div className="rounded-[28px] border border-[#DBEAFE] bg-[#F8FBFF] p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <div className="rounded-[22px] border border-[#BBF7D0] bg-[#F0FDF4] px-4 py-4">
          ...
          <p className="mt-2 text-sm leading-7 text-[#166534]">{insuranceNeedGuidance(weakest?.label || '')}</p>
        </div>
      </div>
    </div>
  );
...
export default function FamilyPolicyReportSheet({ onClose, loadPolicies, customerName, scopeLabel }: Props) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        try {
          response = await api.resolveFamilyPolicyReport(payload);
        } catch (resolveErr: any) {
          if (String(resolveErr?.code || '') !== 'FAMILY_POLICY_REPORT_NOT_FOUND') {
            throw resolveErr;
          }
          response = await api.generateFamilyPolicyReport(payload);
        }
        if (!mounted) return;
        setReport(response);
      } catch (err: any) {
  ...
  }, [customerName, loadPolicies, scopeLabel]);
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[70] flex flex-col bg-[linear-gradient(180deg,#EAF4FB_0%,#F4F9FD_34%,#F8FBFE_100%)]"
    >
        ...
        <div className="space-y-6">
          <section className="overflow-hidden rounded-[38px] border border-[#BFDBFE] bg-white shadow-[0_36px_80px_-54px_rgba(3,105,161,0.52)]">
            <div className="border-b border-[#DBEAFE] px-6 py-6 md:px-8 md:py-8">
              ...
              <div className="mt-6 flex flex-wrap items-start justify-between gap-5">
                <div className="max-w-3xl">
                  <p className="text-[11px] font-bold uppercase tracking-[0.32em] text-[#0369A1]">家庭保单结构总览</p>
                  <h2 className="mt-4 text-[36px] font-black leading-[1.12] text-[#0C4A6E] md:text-[46px]" style={SERIF_STYLE}>
                    从家庭结构、投保人关系和保单内容三条线，把这组保障讲清楚。
                  </h2>
                  <p className="mt-4 max-w-2xl text-[15px] leading-8 text-[#335270]">
                    这份报告先把当前已录入的全部保单按家庭成员和投保人关系整理成结构图，再按保障类型、缴费节奏和保单内容做归档展示，最后给出书面结论。
                  </p>
                </div>
                ...
                <div className="min-w-[240px] rounded-[28px] border border-[#DBEAFE] bg-[#F8FBFF] px-5 py-4 shadow-sm">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-2xl border border-[#DBEAFE] bg-white px-3 py-2">
                      ...
                      <p className="mt-1 font-semibold text-[#0C4A6E]">{scopeLabel || '当前已录入保单'}</p>
                    </div>
                    <div className="rounded-2xl border border-[#DBEAFE] bg-white px-3 py-2">
            ...
            <div className="grid gap-3 px-6 py-6 md:grid-cols-4 md:px-8">
              ...
              <SummaryMetric label="年度保费" value={formatCurrency(totalPremium)} note="用于观察当前家庭缴费压力和节奏" />
              <SummaryMetric label="合计保额" value={formatCoverageAmount(totalCoverage)} note="用于观察家庭保障厚度和集中度" />
            </div>
          </section>
          ...
          <ChapterShell
            ...
            note="先看谁在投保、为谁投保、集中在哪些保障类型，便于理解家庭保单是如何被配置出来的。"
          >
          ...
          <section className="rounded-[34px] border border-[#BFDBFE] bg-white shadow-[0_28px_60px_-48px_rgba(3,105,161,0.45)]">
            <div className="grid gap-0 md:grid-cols-[240px_minmax(0,1fr)]">
              <aside className="border-b border-[#DBEAFE] bg-[#F8FBFF] px-6 py-6 md:border-b-0 md:border-r">
                ...
                <p className="mt-3 text-sm leading-7 text-[#53718F]">这部分说明当前报告是如何被整理出来的，避免把“有保单”直接误解成“保障完整”。</p>
              </aside>
          ...
          {!loading && !error && !policies.length ? (
            <section className="rounded-[34px] border border-[#BFDBFE] bg-white p-6 shadow-[0_28px_60px_-48px_rgba(3,105,161,0.45)]">
              ...
              <p className="mt-3 text-sm leading-7 text-[#53718F]">先录入至少一张保单，再回来查看家庭保障分析报告。</p>
            </section>
          ) : null}
