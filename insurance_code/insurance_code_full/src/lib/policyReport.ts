...
function inferProtectionType(policy: InsurancePolicy) {
...
function policyKeywordText(policy: InsurancePolicy) {
  return `${policy.company} ${policy.type} ${policy.name} ${(policy.responsibilities || []).map((item) => `${item.name} ${item.desc || ''}`).join(' ')}`;
}
...
export function buildInsuranceNeedAxes(policies: InsurancePolicy[]): InsuranceNeedAxis[] {
  const definitions = [
    {
      dimension: '健康',
      match: (policy: InsurancePolicy) => {
        const text = policyKeywordText(policy);
        return /医疗|住院|门急诊|百万|重疾|疾病|防癌/.test(text);
      },
    },
    {
      dimension: '人寿',
      match: (policy: InsurancePolicy) => {
        const text = policyKeywordText(policy);
        return /寿险|身故|全残|定期|终身/.test(text);
      },
    },
    {
      dimension: '养老',
      match: (policy: InsurancePolicy) => {
        const text = policyKeywordText(policy);
        return /养老|年金|退休|增额|终身寿/.test(text);
      },
    },
    {
      dimension: '财富',
      match: (policy: InsurancePolicy) => {
        const text = policyKeywordText(policy);
        return /财富|教育|传承|理财|分红|万能|增额|年金/.test(text);
      },
    },
    {
      dimension: '意外',
      match: (policy: InsurancePolicy) => {
        const text = policyKeywordText(policy);
        return /意外|驾乘|交通/.test(text);
      },
    },
  ];

  const axes = definitions.map((definition) => {
    const matched = policies.filter(definition.match);
    const coverage = matched.reduce((sum, policy) => sum + Number(policy.amount || 0), 0);
    const annualPremium = matched.reduce((sum, policy) => sum + Number(policy.annualPremium || 0), 0);
    const policyCount = matched.length;
    const raw = coverage / 100000 + annualPremium / 5000 + policyCount * 1.2;
    return {
  ...
  const maxRaw = Math.max(...axes.map((item) => item.raw), 1);
  return axes.map((item) => ({
    dimension: item.dimension,
    score: item.raw <= 0 ? 0 : Math.max(18, Math.min(100, Math.round((item.raw / maxRaw) * 100))),
    coverage: item.coverage,
    annualPremium: item.annualPremium,
    policyCount: item.policyCount,
  }));
...
export function buildFamilyPolicyReportPayload(policies: InsurancePolicy[], options: BuildFamilyPolicyReportOptions = {}) {
  ...
  return {
    ...
    family_profile: {
      family_name: safeText(options.customerName, '我的家庭'),
      scope_label: safeText(options.scopeLabel, '当前已录入保单'),
