...
function writeReports(report) {
  ...
  const jsonPath = path.join(REPORT_DIR, `week15-activity-pilot-${stamp}.json`);
  const mdPath = path.join(REPORT_DIR, `week15-activity-pilot-${stamp}.md`);
  const latestJsonPath = path.join(REPORT_DIR, 'week15-activity-pilot-latest.json');
  const latestMdPath = path.join(REPORT_DIR, 'week15-activity-pilot-latest.md');
...
async function main() {
  ...
  try {
    ...
    assert(monolithReady?.status === 200, 'week15 monolith health failed', monolithReady);
    const userReady = await waitForReady(userService.base, '/ready');
    assert(userReady?.status === 200, 'week15 user-service ready failed', userReady);
    const pointsReady = await waitForReady(pointsService.base, '/ready');
    assert(pointsReady?.status === 200, 'week15 points-service ready failed', pointsReady);
    const activityReady = await waitForReady(activityService.base, '/ready');
    assert(activityReady?.status === 200, 'week15 activity-service ready failed', activityReady);
    const gatewayReady = await waitForReady(gateway.base, '/ready');
    assert(gatewayReady?.status === 200, 'week15 gateway ready failed', gatewayReady);
    ...
    checks.push(buildCheck('activity.routes.registered.week15', routes, routes.status === 200 && routeList.includes('/api/activities/:id/complete'), {
    ...
    const createViaGateway = async (title, traceId, expectedTarget) => {
      const response = await requestJson(gateway.base, '/api/p/activities', {
        ...
        body: {
          ...
          category: 'task',
          rewardPoints: 9,
          content: `week15 runtime split ${title}`,
          status: 'online',
        },
      });
    ...
    const sendCode = await requestJson(gateway.base, '/api/auth/send-code', {
      method: 'POST',
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week15-activity-user-send-code' }),
      body: { mobile },
    });
    assert(sendCode.status === 200, 'week15 activity send-code failed', sendCode);

    const verifyBasic = await requestJson(gateway.base, '/api/auth/verify-basic', {
      method: 'POST',
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week15-activity-user-login' }),
      body: {
        name: '活动客户',
        mobile,
    ...
    assert(verifyBasic.status === 200 && customerToken && customerCsrfToken && customerId > 0, 'week15 activity verify-basic failed', verifyBasic);
    const customerRow = (getState().users || []).find((row) => Number(row.id || 0) === customerId);
    assert(customerRow, 'week15 activity customer row missing after login', { customerId });
    ...
    const { activityId: defaultActivityId } = await createViaGateway(`Week15-activity-v1-${Date.now()}`, 'week15-activity-admin-create-v1', 'v1-monolith');

    const defaultRead = await requestJson(gateway.base, '/api/activities', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-default-read-v1',
    ...
    const pointsBeforeDefault = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-points-before-default',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    assert(pointsBeforeDefault.status === 200, 'week15 points before default complete failed', pointsBeforeDefault);
    ...
    const defaultComplete = await requestJson(gateway.base, `/api/activities/${defaultActivityId}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-v1',
        authHeaders: {
    ...
    const pointsAfterDefault = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-points-after-default',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    assert(pointsAfterDefault.status === 200, 'week15 points after default complete failed', pointsAfterDefault);
    ...
    const { activityId: v2ActivityId, rewardPoints: v2RewardPoints, response: createV2 } = await createViaGateway(
      `Week15-activity-v2-${Date.now()}`,
      'week15-activity-admin-create-v2',
      'activity-service',
    );
    ...
    const cutoverRead = await requestJson(gateway.base, '/api/activities', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-cutover-read-v2',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    ...
    const pointsBeforeV2 = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-points-before-v2',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    assert(pointsBeforeV2.status === 200, 'week15 points before v2 complete failed', pointsBeforeV2);
    ...
    const completeV2 = await requestJson(gateway.base, `/api/activities/${v2ActivityId}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-v2',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    const pointsAfterV2 = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-points-after-v2',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    ...
    assert(pointsAfterV2.status === 200 && rewardDelta === v2RewardPoints, 'week15 points summary did not reflect activity reward', {
      pointsBeforeV2,
      pointsAfterV2,
      rewardDelta,
      expectedReward: v2RewardPoints,
    ...
    const rewardLog = findRecentPointsLog(pointsObservability.body, 'week15-activity-complete-v2');
    ...
    assert(rewardLog, 'week15 points observability missing activity reward trace', { pointsObservability });

    const gatewayMetricsAfterV2 = await requestJson(gateway.base, '/internal/gateway/metrics');
    const gatewayV2CompleteEntry = findRecentGatewayEntry(gatewayMetricsAfterV2.body, 'week15-activity-complete-v2');
    ...
    assert(gatewayV2CompleteEntry, 'week15 gateway metrics missing v2 complete trace', { gatewayMetricsAfterV2 });

    process.env.GATEWAY_FORCE_V1_PATHS = '/api/activities,/api/p/activities,/api/b/activity-configs';
    const { activityId: forceV1ActivityId } = await createViaGateway(`Week15-activity-force-v1-${Date.now()}`, 'week15-activity-admin-create-force-v1', 'v1-monolith');
    const forceV1Complete = await requestJson(gateway.base, `/api/activities/${forceV1ActivityId}/complete`, {
      method: 'POST',
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-force-v1',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(forceV1Complete.status === 200, 'week15 activity force-v1 failed', forceV1Complete);

    process.env.GATEWAY_FORCE_V1_PATHS = '';
    const { activityId: backV2ActivityId } = await createViaGateway(`Week15-activity-back-v2-${Date.now()}`, 'week15-activity-admin-create-back-v2', 'activity-service');
    const backToV2 = await requestJson(gateway.base, `/api/activities/${backV2ActivityId}/complete`, {
      method: 'POST',
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-back-v2',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(backToV2.status === 200, 'week15 activity back-to-v2 failed', backToV2);

    const { activityId: rollbackActivityId } = await createViaGateway(`Week15-activity-manual-rollback-${Date.now()}`, 'week15-activity-admin-create-manual-rollback', 'activity-service');
    ...
    const readFallback = await requestJson(gateway.base, '/api/activities', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-read-fallback',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    ...
    assert(readFallback.status === 200, 'week15 activity read fallback failed', readFallback);
    ...
    const writeNoAutoFallback = await requestJson(gateway.base, `/api/activities/${rollbackActivityId}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-no-auto-fallback',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(writeNoAutoFallback.status === 502, 'week15 activity write path should not auto fallback', writeNoAutoFallback);
    ...
    const manualRollback = await requestJson(gateway.base, `/api/activities/${rollbackActivityId}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week15-activity-complete-manual-rollback',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(manualRollback.status === 200, 'week15 activity manual rollback failed', manualRollback);

    const gatewayMetricsAfterManualRollback = await requestJson(gateway.base, '/internal/gateway/metrics');
    const gatewayFallbackEntry = findRecentGatewayEntry(gatewayMetricsAfterManualRollback.body, 'week15-activity-read-fallback');
