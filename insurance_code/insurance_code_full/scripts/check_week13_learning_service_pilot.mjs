...
async function main() {
  ...
  assert(diff.missing.length === 0, 'gateway missing learning-service pilot routes', diff);
  assert(diff.unexpected.length === 0, 'gateway has unexpected learning-service pilot routes', diff);
  ...
  assert(!gatewayLearningRoutes.includes('/api/learning/courses/:id/complete'), 'learning complete route must stay on v1-monolith', {
    gatewayLearningRoutes,
  });
  assert(!gatewayLearningRoutes.includes('/api/learning/games'), 'learning games route must stay on v1-monolith in Week13 pilot', {
    gatewayLearningRoutes,
  });
  assert(!gatewayLearningRoutes.includes('/api/learning/tools'), 'learning tools route must stay on v1-monolith in Week13 pilot', {
    gatewayLearningRoutes,
  });
