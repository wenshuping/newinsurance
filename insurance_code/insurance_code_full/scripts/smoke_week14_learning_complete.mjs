...
function toMarkdown(report) {
  ...
  lines.push(
    ...
    '## 结论',
    '',
    `- default learning traffic on v1: ${report.criteria.defaultLearningTrafficOnV1 ? 'PASS' : 'FAIL'}`,
...
function writeReports(report) {
  ...
  const jsonPath = path.join(REPORT_DIR, `week14-learning-complete-${stamp}.json`);
  const mdPath = path.join(REPORT_DIR, `week14-learning-complete-${stamp}.md`);
  const latestJsonPath = path.join(REPORT_DIR, 'week14-learning-complete-latest.json');
  const latestMdPath = path.join(REPORT_DIR, 'week14-learning-complete-latest.md');
...
async function main() {
  ...
  try {
    ...
    assert(monolithReady?.status === 200, 'week14 monolith health failed', monolithReady);
    const userReady = await waitForReady(userService.base, '/ready');
    assert(userReady?.status === 200, 'week14 user-service ready failed', userReady);
    const pointsReady = await waitForReady(pointsService.base, '/ready');
    assert(pointsReady?.status === 200, 'week14 points-service ready failed', pointsReady);
    const learningReady = await waitForReady(learningService.base, '/ready');
    assert(learningReady?.status === 200, 'week14 learning-service ready failed', learningReady);
    const gatewayReady = await waitForReady(gateway.base, '/ready');
    assert(gatewayReady?.status === 200, 'week14 gateway ready failed', gatewayReady);
    ...
    checks.push(buildCheck('learning.routes.registered.week14', routes, routes.status === 200 && routeList.includes('/api/learning/courses/:id/complete'), {
      routeCount: routeList.length,
    }));
    assert(routes.status === 200, 'gateway route list failed', routes);
    assert(routeList.includes('/api/learning/courses/:id/complete'), 'gateway did not register learning complete route', { routeList });

    const defaultRead = await requestJson(gateway.base, '/api/learning/courses', {
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week14-learning-default-read-v1' }),
    });
    checks.push(buildCheck('learning.default.read.v1', defaultRead, defaultRead.status === 200 && defaultRead.headers['x-gateway-target-service'] === 'v1-monolith'));
    assert(defaultRead.status === 200, 'learning default read failed', defaultRead);

    process.env.GATEWAY_ENABLE_LEARNING_SERVICE = 'true';

    const cutoverRead = await requestJson(gateway.base, '/api/learning/courses', {
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week14-learning-cutover-read-v2' }),
    });
    checks.push(buildCheck('learning.cutover.read.v2', cutoverRead, cutoverRead.status === 200 && cutoverRead.headers['x-gateway-target-service'] === 'learning-service'));
    assert(cutoverRead.status === 200, 'learning cutover read failed', cutoverRead);
    ...
    const createCourse = async (title, traceId) => {
      const response = await requestJson(gateway.base, '/api/p/learning/courses', {
        ...
        body: {
          ...
          contentType: 'article',
          rewardPoints: 12,
          content: `week14 runtime split ${title}`,
          status: 'published',
        },
      });
    ...
    const sendCode = await requestJson(gateway.base, '/api/auth/send-code', {
      method: 'POST',
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week14-learning-user-send-code' }),
      body: { mobile },
    });
    assert(sendCode.status === 200, 'week14 learning send-code failed', sendCode);

    const verifyBasic = await requestJson(gateway.base, '/api/auth/verify-basic', {
      method: 'POST',
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week14-learning-user-login' }),
      body: {
        name: '张三',
        mobile,
    ...
    assert(verifyBasic.status === 200 && customerToken && customerCsrfToken && customerId > 0, 'week14 learning verify-basic failed', verifyBasic);
    const customerRow = (getState().users || []).find((row) => Number(row.id || 0) === customerId);
    assert(customerRow, 'week14 learning customer row missing after login', { customerId });
    ...
    const { id: courseV2Id } = await createCourse(`Week14-learning-v2-${Date.now()}`, 'week14-learning-admin-create-v2');

    const customerCourses = await requestJson(gateway.base, '/api/learning/courses', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-user-list',
        authHeaders: {
    ...
    assert(visibleV2Course, 'week14 created course is not visible to assigned customer', { courseV2Id, visibleCourses });

    const courseDetail = await requestJson(gateway.base, `/api/learning/courses/${courseV2Id}`, {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-user-detail',
    ...
    assert(courseDetail.status === 200, 'week14 customer detail failed', courseDetail);

    const pointsBefore = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-points-before',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    assert(pointsBefore.status === 200, 'week14 points summary before complete failed', pointsBefore);
    ...
    const completeV2 = await requestJson(gateway.base, `/api/learning/courses/${courseV2Id}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-complete-v2',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(completeV2.status === 200, 'week14 learning complete v2 failed', completeV2);

    const pointsAfter = await requestJson(gateway.base, '/api/points/summary', {
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-points-after',
        authHeaders: { authorization: `Bearer ${customerToken}` },
      }),
    });
    const rewardDelta = Number(pointsAfter.body?.balance || 0) - balanceBefore;
    checks.push(buildCheck('learning.reward.points-summary-updated', pointsAfter, pointsAfter.status === 200 && rewardDelta === 12, { rewardDelta }));
    assert(pointsAfter.status === 200 && rewardDelta === 12, 'week14 points summary did not reflect learning reward', { pointsBefore, pointsAfter, rewardDelta });
    ...
    const rewardLog = pointsLogs.find((row) => String(row?.route || '') === 'INTERNAL learning->points reward settlement' && String(row?.trace_id || '') === 'week14-learning-complete-v2' && String(row?.result || '') === 'success');
    ...
    assert(rewardLog, 'week14 points observability missing learning reward trace', { pointsObservability });

    const gatewayMetricsAfterV2 = await requestJson(gateway.base, '/internal/gateway/metrics');
    const gatewayV2CompleteEntry = findRecentGatewayEntry(gatewayMetricsAfterV2.body, 'week14-learning-complete-v2');
    checks.push(buildCheck('learning.complete.gateway-metrics.v2', gatewayMetricsAfterV2, gatewayMetricsAfterV2.status === 200 && gatewayV2CompleteEntry?.target_service === 'learning-service', {
      fallbackCount: Number(gatewayV2CompleteEntry?.fallback_count || 0),
    }));
    assert(gatewayV2CompleteEntry, 'week14 gateway metrics missing v2 complete trace', { gatewayMetricsAfterV2 });

    process.env.GATEWAY_FORCE_V1_PATHS = '/api/learning/courses';
    const { id: courseForceV1Id } = await createCourse(`Week14-learning-force-v1-${Date.now()}`, 'week14-learning-admin-create-force-v1');
    const completeForceV1 = await requestJson(gateway.base, `/api/learning/courses/${courseForceV1Id}/complete`, {
      method: 'POST',
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-complete-force-v1',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(completeForceV1.status === 200, 'week14 learning complete force-v1 failed', completeForceV1);
    ...
    const forceV1RewardLog = pointsLogsAfterForceV1.find(
      (row) => String(row?.route || '') === 'INTERNAL learning->points reward settlement'
        && String(row?.trace_id || '') === 'week14-learning-complete-force-v1'
    ...
    assert(forceV1RewardLog, 'week14 force-v1 complete did not use points-service reward contract', {
      pointsObservabilityAfterForceV1,
    });

    process.env.GATEWAY_FORCE_V1_PATHS = '';
    const { id: courseBackV2Id } = await createCourse(`Week14-learning-back-v2-${Date.now()}`, 'week14-learning-admin-create-back-v2');
    const completeBackV2 = await requestJson(gateway.base, `/api/learning/courses/${courseBackV2Id}/complete`, {
      method: 'POST',
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-complete-back-v2',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(completeBackV2.status === 200, 'week14 learning complete back-to-v2 failed', completeBackV2);

    const { id: courseManualRollbackId } = await createCourse(`Week14-learning-write-rollback-${Date.now()}`, 'week14-learning-admin-create-manual-rollback');
    process.env.GATEWAY_LEARNING_SERVICE_URL = 'http://127.0.0.1:9';

    const readFallback = await requestJson(gateway.base, '/api/learning/courses', {
      headers: buildGatewayHeaders({ tenantId, tenantCode: 'tenant-alpha', traceId: 'week14-learning-read-fallback' }),
    });
    checks.push(buildCheck('learning.read.fallback.v1', readFallback, readFallback.status === 200 && readFallback.headers['x-gateway-target-service'] === 'v1-monolith'));
    assert(readFallback.status === 200, 'week14 learning read fallback failed', readFallback);
    ...
    const writeNoAutoFallback = await requestJson(gateway.base, `/api/learning/courses/${courseManualRollbackId}/complete`, {
      ...
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-complete-no-auto-fallback',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    assert(writeNoAutoFallback.status === 502, 'week14 learning write path should not auto fallback', writeNoAutoFallback);

    process.env.GATEWAY_FORCE_V1_PATHS = '/api/learning/courses';
    const manualRollback = await requestJson(gateway.base, `/api/learning/courses/${courseManualRollbackId}/complete`, {
      method: 'POST',
      headers: buildGatewayHeaders({
        tenantId,
        tenantCode: 'tenant-alpha',
        traceId: 'week14-learning-complete-manual-rollback',
        authHeaders: {
          authorization: `Bearer ${customerToken}`,
          'x-csrf-token': customerCsrfToken,
        },
    ...
    checks.push(buildCheck('learning.complete.manual-rollback.v1', manualRollback, manualRollback.status === 200 && manualRollback.headers['x-gateway-target-service'] === 'v1-monolith', { courseId: courseManualRollbackId }));
    assert(manualRollback.status === 200, 'week14 learning manual rollback failed', manualRollback);

    const pointsObservabilityAfterManualRollback = await requestJson(pointsService.base, '/internal/points-service/observability');
    const pointsLogsAfterManualRollback = Array.isArray(pointsObservabilityAfterManualRollback.body?.recentLogs)
      ? pointsObservabilityAfterManualRollback.body.recentLogs
      : [];
    const manualRollbackRewardLog = pointsLogsAfterManualRollback.find(
      (row) => String(row?.route || '') === 'INTERNAL learning->points reward settlement'
        && String(row?.trace_id || '') === 'week14-learning-complete-manual-rollback'
        && String(row?.result || '') === 'success',
    );
    checks.push(
      buildCheck(
        'learning.reward.points-observability.manual-rollback',
        pointsObservabilityAfterManualRollback,
        pointsObservabilityAfterManualRollback.status === 200 && Boolean(manualRollbackRewardLog),
      ),
    );
    assert(manualRollbackRewardLog, 'week14 manual rollback complete did not use points-service reward contract', {
      pointsObservabilityAfterManualRollback,
    });

    const gatewayMetricsAfterFallback = await requestJson(gateway.base, '/internal/gateway/metrics');
    const readFallbackEntry = findRecentGatewayEntry(gatewayMetricsAfterFallback.body, 'week14-learning-read-fallback');
    const manualRollbackEntry = findRecentGatewayEntry(gatewayMetricsAfterFallback.body, 'week14-learning-complete-manual-rollback');
    checks.push(buildCheck('learning.complete.gateway-metrics.rollback', gatewayMetricsAfterFallback, gatewayMetricsAfterFallback.status === 200 && manualRollbackEntry?.target_service === 'v1-monolith' && readFallbackEntry?.fallback_count === 1, {
      readFallbackCount: Number(readFallbackEntry?.fallback_count || 0),
      manualRollbackTarget: manualRollbackEntry?.target_service || null,
    }));
    assert(readFallbackEntry, 'week14 gateway metrics missing read fallback trace', { gatewayMetricsAfterFallback });
    assert(manualRollbackEntry, 'week14 gateway metrics missing manual rollback trace', { gatewayMetricsAfterFallback });
    ...
    const report = {
      ...
      checks,
      criteria: {
        defaultLearningTrafficOnV1: checks.some((item) => item.name === 'learning.default.read.v1' && item.ok),
        completeCutoverToV2: checks.some((item) => item.name === 'learning.complete.v2' && item.ok),
        completeRewardChainWorks: checks.some((item) => item.name === 'learning.reward.points-summary-updated' && item.ok) && checks.some((item) => item.name === 'learning.reward.points-observability' && item.ok),
        ...
        manualWriteRollbackWorks: checks.some((item) => item.name === 'learning.complete.manual-rollback.v1' && item.ok),
        gatewayMetricsCoverComplete: checks.some((item) => item.name === 'learning.complete.gateway-metrics.v2' && item.ok) && checks.some((item) => item.name === 'learning.complete.gateway-metrics.rollback' && item.ok),
        pointsObservabilityCoverReward:
          ...
          && checks.some((item) => item.name === 'learning.reward.points-observability.manual-rollback' && item.ok),
      },
    };
