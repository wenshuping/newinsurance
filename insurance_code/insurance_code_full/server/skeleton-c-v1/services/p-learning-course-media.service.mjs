...
const toUploadedLearningMediaItem = (file, fallback = {}) => {
...
export async function resolvePLearningCourseMedia({ media = [], uploadItems = [], uploadFile, limit = 6 } = {}) {
  ...
  for (const item of normalizedUploads.slice(0, capacity)) {
    ...
    uploadedMedia.push(toUploadedLearningMediaItem(uploaded, item));
  }
